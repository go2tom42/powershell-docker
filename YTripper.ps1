...
foreach($key in $FileContent.keys)
{
    ...
    if ($FileContent[$key]["lastid"].Equals("")) {
        ...
        $arguments = '-i --add-metadata  --no-warnings --ignore-config --write-sub --embed-subs --ignore-errors --retries 16 --download-archive archive.txt --external-downloader aria2c --external-downloader-args "--file-allocation=none -c -j 5 -x '+$aria2cThreads+' -s '+$aria2cThreads+' -k 1M"  -f "bestvideo[height<=?'+$res+'][ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best" -o "'+$path+''+$os+'%(upload_date)s - %(title)s[%(id)s].%(ext)s" ' +$DNurl
